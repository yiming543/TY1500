...
# 332 "./mcc_generated_files/pin_manager.h"
...
void check_turn_signals(void) {
  ...
  if (ORB.TURN) {
    turn_on_cnt++;
    if (turn_on_cnt >= TURN_LIGHT_ON_600MS)
    {
      fWelcome_light = 1;
...
void all_white_led_turn_on(void) { LATCbits.LATC3 = ON; }
void all_white_led_turn_off(void) { LATCbits.LATC3 = OFF; }
...
void check_welcome_light(void) {
  ...
  case WELCOME_STEP_13:
    yellow_to_white();
    _delay((unsigned long)((II_CNT)*(32000000/4000.0)));
    _delay((unsigned long)((II_CNT)*(32000000/4000.0)));
    eWelcome_step++;
    break;
