...
void CheckTurnSignals(void) {
  ...
  if (!ORB.TURN) {
    TurnOff_cnt++;
    if (TurnOff_cnt >= TURN_LIGHT_OFF_600MS)
    {
  ...
  if (ORB.TURN) {
    TurnON_cnt++;
    if (TurnON_cnt >= TURN_LIGHT_ON_600MS)
    {
      fWelcomeLight = 1;
...
void WLED_ALL_ON(void) { LATCbits.LATC3 = ON; }
void WLED_ALL_OFF(void) { LATCbits.LATC3 = OFF; }
